...
import jos3
from PMV import pmv_ppd
from SET import set_tmp  # 新增：SET 计算函数
import numpy as np
import pandas as pd
...
# 注意这里显式转 str，避免某些版本对 PathLike 不兼容
...
def api_pmv(payload: PMVInput):
    ...
    return PMVOutput(ok=True, pmv=pmv_value, ppd=ppd_value)
...
def api_set(payload: SETInput):
    ...
    # 注意：set_tmp 的参数名是 (tdb, tr, v, rh, met, clo, wme, body_surface_area, p_atm, position)
    value = set_tmp(
        payload.ta,                   # tdb
        tr,                           # tr
        payload.vel,                  # v
    ...
    # set_tmp 可能返回 numpy 标量，这里统一转成 float
    value = float(value)
